...
import type {
    AdminServicePageItem as Service,
    PaginationInfo,
    ServiceDetail,
} from "@/shared/types/service";
import type { ServiciosEstadisticas } from "@/features/services/api";
import { useAdminTheme } from "@/features/admin/hooks/useAdminTheme";
import ServiciosDashboard from "@/features/admin/servicios/ServiciosDashboard";
...
function Cell(
    { label, value, dark }: { label: string; value: string; dark: boolean },
) {
    return (
        <div>
            <p
...
export default function ServicesPageClient({
    ...
    const [lookedUpService, setLookedUpService] = useState<
        ServiceDetail | null
    >(null);
    ...
    const handleStatusChange = useCallback((id: number, newEstado: string) => {
        ...
        const prevEstado = servicesRef.current.find((s) =>
            s.noDeServicio === id
        )?.estado;
        setServices((prev) =>
            prev.map((s) =>
                s.noDeServicio === id
                    ? { ...s, estado: newEstado as Service["estado"] }
                    : s
            )
        );
        updateServiceStatusAction(String(id), newEstado).then((result) => {
            if (!result.success && prevEstado !== undefined) {
                ...
                setServices((prev) =>
                    prev.map((s) =>
                        s.noDeServicio === id
                            ? { ...s, estado: prevEstado as Service["estado"] }
                            : s
                    )
                );
            }
    ...
    const handleRevisionChange = useCallback(
        (id: number, newRevision: string) => {
            const prevRevision = servicesRef.current.find((s) =>
                s.noDeServicio === id
            )?.estadoPago;
            setServices((prev) =>
                prev.map((s) =>
                    s.noDeServicio === id
                        ? {
                            ...s,
                            estadoPago: newRevision as Service["estadoPago"],
                        }
                        : s
                )
            );
            updateRevisionStatusAction(String(id), newRevision).then(
                (result) => {
    ...
    const aparatos = Array.from(
        new Set(services.map((s) => s.aparato).filter(Boolean)),
    ).sort();
    ...
    return (
        <div className={`${dark ? "text-gray-200" : "text-gray-900"}`}>
                ...
                <div className="container mx-auto px-4 py-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            ...
                            >
                                Administra todos los servicios de reparación y
                                mantenimiento
                            </p>
                        </div>
            ...
            <div className="container mx-auto px-4 py-6 space-y-4">
                ...
                <ServiciosDashboard estadisticas={estadisticas} />

                {/* Lookup by ID */}
                <div
                    className={`${
                        dark
                            ? "rounded-lg border border-white/10 bg-[#0F172A] shadow-sm shadow-black/20"
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                                <Cell
                                    dark={dark}
                                    label="No. Servicio"
                                    value={`#${lookedUpService.noDeServicio}`}
                                />
                                <Cell
                                    dark={dark}
                                    label="Fecha"
                                    value={formatDate(lookedUpService.fecha)}
                                />
                                <Cell
                                    dark={dark}
                                    label="Cliente"
                                    value={lookedUpService.cliente}
                                />
                                <Cell
                                    dark={dark}
                                    label="Aparato"
                                    value={lookedUpService.aparato}
                                />
                                <Cell
                                    dark={dark}
                                    label="Teléfono"
                                    value={formatPhone(
                                        lookedUpService.telefono,
                                    )}
                                />
                                <Cell
                                    dark={dark}
                                    label="Estado"
                                    value={lookedUpService.estado}
                                />
                            </div>
                            <Link
