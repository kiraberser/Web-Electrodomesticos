...
import { registrarSalidaAction } from "@/features/admin/inventario-actions"
import type { Refaccion, Categoria } from "@/features/catalog/api"
import { ArrowUpFromLine, X, MapPin } from "lucide-react"
...
export default function SalidaForm({ refacciones, categorias, onSuccess, onCancel }: SalidaFormProps) {
    const { dark } = useAdminTheme()
    const [state, formAction] = useActionState(registrarSalidaAction, { success: false, error: null })
                ...
                >
                    ...
                    {filteredRefacciones.map((r) => (
                        <option key={r.id} value={r.id}>
                            {r.nombre} ({r.codigo_parte}) - Stock: {r.existencias}
                        </option>
                    ))}
                </Select>
            </div>
                ...
                <div className={`flex items-center gap-3 rounded-lg border p-3 ${dark ? 'border-slate-700 bg-slate-800/50' : 'border-gray-200 bg-gray-50'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs ${dark ? 'text-slate-400' : 'text-gray-500'}`}>
                            Stock disponible: <strong className={dark ? 'text-slate-200' : 'text-gray-900'}>{selectedRef.existencias}</strong> unidades &middot; {selectedRef.marca}
                        </p>
